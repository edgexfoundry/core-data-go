...
func encode(i interface{}, w http.ResponseWriter) {
	...
	// Problems encoding
	if err != nil {
		log.Error("Error encoding the data: " + err.Error())
...
func checkDevice(device string) bool {
	...
	if err != nil {
		...
		if err != nil {
			log.Error("Can't find device: " + device)
			return false
		}
		return true
	}
...
func putEventOnQueue(e models.Event) {
	log.Info("Putting event on message queue", "")
	//	Have multiple implementations (start with ZeroMQ)
	err := ep.SendEventMessage(e)
	if err != nil {
		log.Error("Unable to send message for event: " + e.String())
	}
}
...
func updateDeviceLastReportedConnected(device string) {
	...
	if !configuration.Deviceupdatelastconnected {
		log.Debug("Skipping update of device connected/reported times for:  " + device)
		return
	}
	...
	if err != nil {
		log.Error("Error getting device " + device + ": " + err.Error())
		return
	}
	...
	if &d == nil {
		...
		if &d == nil {
			log.Error("Error updating device connected/reported times.  Unknown device with identifier of:  " + device)
			return
		}
		...
		if err != nil {
			log.Error("Problems updating last connected value for device: " + d.Id.Hex())
			return
		}
		err = mdc.UpdateLastReported(d.Id.Hex(), t)
		if err != nil {
			log.Error("Problems updating last reported value for device: " + d.Id.Hex())
		}
		return
	}
	...
	if err != nil {
		log.Error("Problems updating last connected value for device: " + d.Name)
		return
	}
	err = mdc.UpdateLastReportedByName(d.Name, t)
	if err != nil {
		log.Error("Problems updating last reported value for device: " + d.Name)
	}
	return
}
...
func updateDeviceServiceLastReportedConnected(device string) {
	if !configuration.Serviceupdatelastconnected {
		log.Debug("Skipping update of device service connected/reported times for:  " + device)
		return
	}
	...
	s := d.Service
	if &s == nil {
		log.Error("Error updating device service connected/reported times.  Unknown device service in device:  " + d.Id.Hex())
		return
	}
...
func eventHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		if err != nil {
			http.Error(w, err.Error(), http.StatusServiceUnavailable)
			log.Error("Error decoding event: " + err.Error())
			return
		}

		log.Info("Posting Event: " + e.String())
		...
		if configuration.Metadatacheck && !deviceFound {
			log.Error("Device not found for event: "+err.Error(), "")
	...
	case "PUT":
		...
		if err != nil {
			http.Error(w, err.Error(), http.StatusServiceUnavailable)
			log.Error("Error decoding the event: " + err.Error())
			return
		}
		...
		log.Info("Updating event: " + from.ID.Hex())
		...
		if from.Device != "" {
			...
			if configuration.Metadatacheck && !deviceFound {
				...
				log.Error("Error updating device, device " + from.Device + " doesn't exist")
				return
			}
...
func eventCountByDeviceIdHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Problem unescaping URL: " + err.Error())
		return
	}
...
func eventIdHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "PUT":
		...
		log.Info("Updating event: " + e.ID.Hex())
	...
	case "DELETE":
		...
		log.Info("Deleting event: " + e.ID.Hex())
...
func getEventByDeviceHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping URL: " + err.Error())
		return
	}
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error converting to integer: " + err.Error())
		return
	}
	...
	if configuration.Metadatacheck && !deviceFound {
		...
		log.Error("Error getting readings for a device: The device doesn't exist")
		return
	}
...
func deleteByDeviceIdHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping the URL: " + err.Error())
		return
	}
	...
	case "DELETE":
		...
		log.Info("Deleting the events for device: " + deviceId)
...
func eventByCreationTimeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Problem converting start time: " + err.Error())
		return
	}
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Problem converting end time: " + err.Error())
		return
	}
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Problem converting limit: " + strconv.Itoa(limit))
		return
	}
...
func readingByDeviceFilteredValueDescriptor(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Problem unescaping value descriptor: " + err.Error())
		return
	}
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Problem unescaping device ID: " + err.Error())
		return
	}
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Problem converting limit to integer: " + err.Error())
		return
	}
	switch r.Method {
	case "GET":
...
func eventByAgeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error converting the age to an integer")
		return
	}
	...
	case "DELETE":
		...
		log.Info("Deleting events by age: " + vars["age"])
...
func scrubHandler(w http.ResponseWriter, r *http.Request) {
	...
	switch r.Method {
	case "DELETE":
		log.Info("Scrubbing events.  Deleting all events that have been pushed")
...
func scrubAllHandler(w http.ResponseWriter, r *http.Request) {
	...
	switch r.Method {
	case "DELETE":
		log.Info("Deleting all events from database")

		err := dbc.ScrubAllEvents()
		if err != nil {
			http.Error(w, err.Error(), http.StatusServiceUnavailable)
			log.Error("Error scrubbing all events/readings: " + err.Error())
			return
		}
...
func readingHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		if err != nil {
			http.Error(w, err.Error(), http.StatusServiceUnavailable)
			log.Error("Error decoding the reading: " + err.Error())
			return
		}
...
func readingByDeviceHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error converting the limit to an integer: " + err.Error())
		return
	}
	deviceId, err := url.QueryUnescape(vars["deviceId"])
	// Problems unescaping URL
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping the device ID: " + err.Error())
		return
	}
...
func readingbyValueDescriptorHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping value descriptor name: " + err.Error())
		return
	}
	limit, err := strconv.Atoi(vars["limit"])
	// Problems converting limit to int
	...
	if err != nil {
		if err == clients.ErrNotFound {
			log.Error("Value Descriptor not found for Reading", "")
...
func readingByUomLabelHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping the UOM Label: " + err.Error())
		return
	}
...
func readingByLabelHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping the label of the value descriptor: " + err.Error())
		return
	}
	limit, err := strconv.Atoi(vars["limit"])
...
func readingByTypeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error escaping the type: " + err.Error())
		return
	}
...
func readingByCreationTimeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error converting the start time to an integer: " + err.Error())
		return
	}
	e, err := strconv.ParseInt((vars["end"]), 10, 64)
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error converting the end time to an integer: " + err.Error())
		return
	}
	l, err := strconv.Atoi(vars["limit"])
	if err != nil {
...
func readingByValueDescriptorAndDeviceHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping the value descriptor name: " + err.Error())
		return
	}
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping the device: " + err.Error())
		return
	}
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error converting limit to an integer: " + err.Error())
		return
	}
	...
	if !checkDevice(device) {
		log.Error("Device not found for reading", "")
...
func valueDescriptorHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		if err != nil {
			http.Error(w, err.Error(), http.StatusServiceUnavailable)
			log.Error("Error decoding the value descriptor: " + err.Error())
			return
		}
		...
		if err != nil {
			http.Error(w, err.Error(), http.StatusServiceUnavailable)
			log.Error("Error checking for format string for POSTed value descriptor")
			return
		}
		if !match {
	...
	case "PUT":
		...
		if from.Formatting != "" {
			...
			if err != nil {
				http.Error(w, err.Error(), http.StatusServiceUnavailable)
				log.Error("Error checking formatting for updated value descriptor")
				return
			}
			if !match {
				http.Error(w, "Value descriptor's format string doesn't fit the required pattern", http.StatusConflict)
				log.Error("Value descriptor's format string doesn't fit the required pattern: " + formatSpecifier)
				return
			}
		...
		if from.Name != "" {
			...
			if from.Name != to.Name {
				...
				if err != nil {
					http.Error(w, err.Error(), http.StatusServiceUnavailable)
					log.Error("Error checking the readings for the value descriptor: " + err.Error())
					return
				}
				...
				if len(r) != 0 {
					...
					log.Error("Data integrity issue.  Value Descriptor with name:  " + from.Name + " is still referenced by existing readings.")
					return
				}
			}
			to.Name = from.Name
...
func valueDescriptorByUomLabelHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping the UOM Label of the value descriptor: " + err.Error())
		return
	}
...
func valueDescriptorByLabelHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, err.Error(), http.StatusServiceUnavailable)
		log.Error("Error unescaping label for the value descriptor: " + err.Error())
		return
	}
...
func valueDescriptorByDeviceHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Error("Device not found: " + err.Error())
		return
	}
...
func pingHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Error("Error writing pong: " + err.Error())
	}
}
...
func heartbeat() {
	// Loop forever
	for true {
		log.Info(configuration.Heartbeatmsg, "")
...
func main() {
	...
	log = logger.NewClient("Core Data", "http://localhost:48061/api/v1/logs")
	log.LogFilePath = "./coredata.log"
	...
	if err != nil {
		log.Error("Connection to Consul could not be made: "+err.Error(), "")
	}
	...
	if err := consulclient.CheckKeyValuePairs(&configuration, configuration.Applicationname, strings.Split(configuration.Consulprofilesactive, ";")); err != nil {
		log.Error("Error getting key/values from Consul: "+err.Error(), "")
	}

	// Update logging based on configuration
	log.RemoteUrl = configuration.Loggingremoteurl
	log.LogFilePath = configuration.Loggingfile
	...
	if err != nil {
		log.Error("Couldn't connect to database: "+err.Error(), "")
		return
	}
	...
	log.Info(configuration.Appopenmsg, "")

	// Time it took to start service
	log.Info("Service started in: "+time.Since(start).String(), "")
	log.Info("Listening on port: " + strconv.Itoa(configuration.Serverport))

	log.Error(http.ListenAndServe(":"+strconv.Itoa(configuration.Serverport), r).Error())
}
